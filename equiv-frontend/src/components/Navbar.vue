<template>
  ...
  >
    ...
    <v-navigation-drawer v-model="drawer" right app false>
      <v-list-item-group>
        <v-list-item>
          <v-list-item-icon>
            <v-icon>mdi-account-cog</v-icon>
    ...
    <v-dialog v-model="logoutAlert" persistent max-width="350">
      <v-card>
        ...
        <v-card-text
          >Todos as suas alterações não guardadas serão perdidas.</v-card-text
        >
        <v-card-actions class="justify-center">
          <v-btn color="green darken-1" text @click="logoutAlert = false"
            >Voltar Atrás</v-btn
          >
          <v-btn color="red darken-1" text @click="handleLogout"
            >Terminar Sessão</v-btn
          >
        </v-card-actions>
      </v-card>
    </v-dialog>
